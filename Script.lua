...
-- ========= [ TAB: Heal (Auto-Heal by HP, fast) ] =========
Tabs.Heal = Window:AddTab({ Title = "Heal", Icon = "heart" })

local ah_toggle = Tabs.Heal:CreateToggle("ah_toggle", { Title = "Auto Heal (by HP%)", Default = false })

local ah_use_mode = Tabs.Heal:CreateDropdown("ah_use_mode", {
    Title = "Use item by",
    Values = {"Name","ItemID"},
    Default = "Name"
})

local ah_item = Tabs.Heal:CreateDropdown("ah_item", {
    Title = "Item name",
    Values = {"Bloodfruit","Berry","Bluefruit","Coconut","Strawberry","Pumpkin","Apple","Lemon","Orange","Banana","Jelly"},
    Default = "Bloodfruit"
})

local ah_itemid = Tabs.Heal:CreateInput("ah_itemid", {
    Title = "ItemID (number)",
    Default = "94", Numeric = true, Finished = true
})

local ah_thresh = Tabs.Heal:CreateSlider("ah_thresh", {
    Title = "Heal below (%)",
    Min = 1, Max = 99, Rounding = 0, Default = 95
})

-- ⚡ новое: скорость и «куски» за заход
local ah_rate = Tabs.Heal:CreateSlider("ah_rate", {
    Title = "Heal rate (s)",
    Min = 0.03, Max = 0.30, Rounding = 2, Default = 0.08
})
local ah_burst = Tabs.Heal:CreateSlider("ah_burst", {
    Title = "Max bites per burst",
    Min = 1, Max = 40, Rounding = 0, Default = 20
})

local ah_debug = Tabs.Heal:CreateToggle("ah_debug", { Title = "Debug logs (F9)", Default = false })

-- чтение HP (как раньше)
local function readHealthPercent()
    ...
    local mg = pg:FindFirstChild("MainGui");    if not mg then return 100 end
    local bars = mg:FindFirstChild("Bars");     if not bars then return 100 end
    local hb = bars:FindFirstChild("Health")
    if hb and hb:IsA("Frame") and hb.Size and hb.Size.X and typeof(hb.Size.X.Scale)=="number" then
        return math.clamp(hb.Size.X.Scale * 100, 0, 100)
    end
    return 100
end

local function resolveHealItem()
    local mode = (ah_use_mode.Value or "Name")
    if mode == "ItemID" then
        local id = tonumber(ah_itemid.Value)
        return nil, id
    else
        local name = ah_item.Value or "Bloodfruit"
        local slot = getSlotByName(name)
        local id   = getItemIdByName(name)
        return slot, id
    end
end
...
task.spawn(function()
    while true do
        task.wait(0.12)
        if not ah_toggle.Value then healLock = false; continue end

        local hp    = readHealthPercent()
        local thr   = ah_thresh.Value or 95
        local band  = 0.5

        if hp < thr and not healLock then
            healLock = true
            task.spawn(function()
                local tries, maxTries = 0, math.max(1, tonumber(ah_burst.Value) or 20)
                local delay = tonumber(ah_rate.Value) or 0.08

                while ah_toggle.Value and tries < maxTries do
                    hp = readHealthPercent()
                    if hp >= (thr - band) then break end

                    local slot, id = resolveHealItem()
                    local ok = false
                    if slot ~= nil then ok = consumeBySlot(slot) end
                    if not ok then ok = consumeById(id) end

                    if ah_debug.Value then
                        print(("[AutoHeal] try=%d hp=%.1f%% slot=%s id=%s -> %s")
                            :format(tries+1, hp, tostring(slot), tostring(id), ok and "USE" or "MISS"))
                    end
...
-- ======= ВИЗУАЛЬНАЯ ПОДСВЕТКА ЗОНЫ A–B =======
local AB_VIS_FOLDER = Workspace:FindFirstChild("_AB_VIS") or Instance.new("Folder", Workspace)
AB_VIS_FOLDER.Name = "_AB_VIS"

local AB_zonePart, AB_zoneBox
local function AB_clearVisual()
    if AB_zoneBox then AB_zoneBox:Destroy(); AB_zoneBox = nil end
    if AB_zonePart then AB_zonePart:Destroy(); AB_zonePart = nil end
end
...
local function AB_updateVisual()
    ...
    local xmin = AB_snap(math.min(a.X, b.X), step)
    local xmax = AB_snap(math.max(a.X, b.X), step)
    local zmin = AB_snap(math.min(a.Z, b.Z), step)
    local zmax = AB_snap(math.max(a.Z, b.Z), step)

    local sizeX = math.max(step, math.abs(xmax - xmin) + step)
    local sizeZ = math.max(step, math.abs(zmax - zmin) + step)

    local y  = (root and root.Position.Y or (a.Y + b.Y)/2) + 0.15
    local cf = CFrame.new((xmin + xmax)/2, y, (zmin + zmax)/2)

    AB_zonePart = Instance.new("Part")
    AB_zonePart.Name = "_AB_ZONE"
    AB_zonePart.Anchored, AB_zonePart.CanCollide, AB_zonePart.CanTouch, AB_zonePart.CanQuery = true, false, false, false
    AB_zonePart.Material = Enum.Material.ForceField
    AB_zonePart.Color = Color3.fromRGB(255,220,80)
    AB_zonePart.Transparency = 0.8
    AB_zonePart.Size = Vector3.new(sizeX, 0.2, sizeZ)
    AB_zonePart.CFrame = cf
    AB_zonePart.Parent = AB_VIS_FOLDER

    AB_zoneBox = Instance.new("SelectionBox")
    AB_zoneBox.LineThickness = 0.03
    AB_zoneBox.Color3 = Color3.fromRGB(255,220,80)
    AB_zoneBox.SurfaceTransparency = 1
    AB_zoneBox.Adornee = AB_zonePart
    AB_zoneBox.Parent  = AB_VIS_FOLDER
end

-- Перерисовывать после респавна
...
BuildTab:CreateButton({ Title="Set Corner A (here)", Callback=function() if root then AB.cornerA = root.Position; print("[AB] A =", AB.cornerA) AB_updateVisual() end end })
BuildTab:CreateButton({ Title="Set Corner B (here)", Callback=function() if root then AB.cornerB = root.Position; print("[AB] B =", AB.cornerB) AB_updateVisual() end end })
BuildTab:CreateButton({ Title="Clear Area (A & B)",  Callback=function() AB.cornerA, AB.cornerB = nil, nil; AB_clearVisual() end })

-- обновление визуала при изменении spacing
