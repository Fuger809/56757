...
-- ========= [ TAB: Rejoin + Auto re-exec ] =========
local TeleportService = game:GetService("TeleportService")
local Players = game:GetService("Players")

-- совместимость с разными экзекуторами
local function getQOT()
    return rawget(getfenv(), "queue_on_teleport")
        or (syn and syn.queue_on_teleport)
        or (fluxus and fluxus.queue_on_teleport)
        or (KRNL_LOADED and queue_on_teleport)
end

-- разбор ссылок (включая share)
local function parseLink(s)
    if type(s) ~= "string" then return end
    s = s:gsub("%%2D","-")
    local code = s:match("roblox%.com/share%?code=([%w]+)")
               or s:match("[?&]code=([%w]+).*[?&]type=Server")
    if code then
        local url = ("https://www.roblox.com/share?code=%s&type=Server"):format(code)
        local ok, html = pcall(function() return game:HttpGetAsync(url) end)
        if ok and type(html)=="string" then
            local deep = html:match("roblox://[^\"'<>%s]+")
            if deep then return parseLink(deep) end
            local pid = html:match('"placeId"%s*:%s*(%d+)')
            local jid = html:match('"gameInstanceId"%s*:%s*"([%w-]+)"')
                    or html:match('"jobId"%s*:%s*"([%w-]+)"')
            if pid then return tonumber(pid), jid end
        end
        return
    end
    local pid, jid = s:match("placeId=(%d+).-[&?]gameInstanceId=([%w-]+)")
    if pid and jid then return tonumber(pid), jid end
    pid = s:match("/games/(%d+)")
    if pid then
        jid = s:match("[?&]jobId=([%w-]+)") or s:match("[?&]gameInstanceId=([%w-]+)")
        return tonumber(pid), jid
    end
    pid = s:match("[?&]placeId=(%d+)")
    if pid then
        jid = s:match("[?&]jobId=([%w-]+)") or s:match("[?&]gameInstanceId=([%w-]+)")
        return tonumber(pid), jid
    end
    pid, jid = s:match("^(%d+)%s+([%w-]+)$")
    if pid and jid then return tonumber(pid), jid end
    pid = s:match("^(%d+)$")
    if pid then return tonumber(pid), nil end
end

Tabs.Rejoin = Window:AddTab({ Title = "Rejoin", Icon = "link" })

-- поле для ссылки / id
local rj_input = Tabs.Rejoin:AddInput("rj_link", {
    Title = "Ссылка / PLACEID JOBID / roblox://",
    Default = ""
})

-- авто-реэкзек: включатель + поле лоадера
local auto_toggle = Tabs.Rejoin:CreateToggle("rj_autoreexec", {
    Title = "Auto re-exec (queue_on_teleport)",
    Default = true
})
local loader_input = Tabs.Rejoin:AddInput("rj_loader", {
    Title = "Лоадер (что запускать на новом сервере)",
    -- СЮДА ВСТАВЬ свой loader. Пример: GitHub/Pastebin RAW на твой большой скрипт:
    Default = [[loadstring(game:HttpGet("https://your.cdn/raw/fuger.lua"))()]]
})

-- функция постановки в очередь
local function queueLoader()
    if not auto_toggle.Value then return true end
    local qot = getQOT()
    if not qot then
        Library:Notify{ Title="Reexec", Content="queue_on_teleport отсутствует в экзекуторе", Duration=5 }
        return false
    end
    local payload = tostring(loader_input.Value or ""):gsub("^%s+","")
    if #payload < 8 then
        Library:Notify{ Title="Reexec", Content="Пустой лоадер. Вставь loadstring(game:HttpGet(\"…\"))()", Duration=5 }
        return false
    end
    -- ставим ПЕРЕД любым Teleport
    qot(payload)
    return true
end

-- дублируем на любой телепорт, если тебя телепортнули извне
Players.LocalPlayer.OnTeleport:Connect(function(state)
    if state == Enum.TeleportState.Started then queueLoader() end
end)

-- кнопки
Tabs.Rejoin:CreateButton({
    Title = "Перезайти на этот же сервер",
    Callback = function()
        queueLoader()
        ...
        if not ok then
            Library:Notify{ Title="Rejoin", Content="Ошибка телепорта: "..tostring(err), Duration=6 }
        end
    end
})

Tabs.Rejoin:CreateButton({
    Title = "Новый сервер этой же игры",
    Callback = function()
        queueLoader()
        local ok, err = pcall(function()
...
Tabs.Rejoin:CreateButton({
    Title = "Перейти по ссылке / id",
    Callback = function()
        local text = tostring(rj_input.Value or "")
        local placeId, jobId = parseLink(text)
        if not placeId then
            Library:Notify{
                Title="Rejoin",
                Content="Не смог разобрать ссылку. Поддерживаются share, roblox://, games/PLACEID?jobId=…, или \"PLACEID JOBID\"",
                Duration=6
            }
            return
        end
